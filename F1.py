...
def update_balance_block():
    balance = 0;
    f = open("BalanceF1.txt", "r")
    #30 from mining a block
    balance = int(f.readline())
    f.close()
    balance=balance+38
    f = open("BalanceF1.txt", "w")
    f.write(str(balance)+"\n")
    f.close()
...
def read_temp(list):
    ...
    for x in list:
        clientSocket.sendto(list[counter].encode(), ('localhost', 15000))
        counter=counter+1
...
def blockChain():
    ...
    #for x in transList:
     #   clientSocket.sendto("F1".encode(), ('localhost', 14000))
      #  clientSocket.sendto(x.encode(), ('localhost', 14000))
    ...
    # send block to server F2
    clientSocket.sendto("F1Block".encode(), ('localhost', 14000))
    clientSocket.sendto(wholeBlock.encode(), ('localhost', 14000))



# will mine the block
def mineBlock():
    print("Full Node 1 is mining a block")
    blockChain()
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if received_transaction == "clientA":
            ...
            clientSocket.sendto("F1".encode(), ('localhost', 14000))
            clientSocket.sendto(received_transaction.encode(), ('localhost', 14000))
            f = open("Tempt_T.txt", "a")
            f.write(received_transaction+"\n")
            f.close()


        #performs instructions if F1 sent the message
        elif received_transaction == "F2":
            message, clientAddress = serverSocket.recvfrom(2048)
            ...
            f.close()
        # performs instructions if F2 sent the message
        elif received_transaction == "F2Block":
            message, clientAddress = serverSocket.recvfrom(2048)
            received_message = message.decode()
            print("Full Node 2 sent blockchain: ")
            print(received_message)
            ...
            clientSocket.sendto(transaction_info.encode(), ('localhost', 15000))
            transaction_info = Tx2[0:8] + ":" + Tx1[8:16] + ":" + Tx1[16:24]
            clientSocket.sendto(transaction_info.encode(), ('localhost', 15000))
            transaction_info = Tx3[0:8] + ":" + Tx1[8:16] + ":" + Tx1[16:24]
            clientSocket.sendto(transaction_info.encode(), ('localhost', 15000))
            transaction_info = Tx4[0:8] + ":" + Tx1[8:16] + ":" + Tx1[16:24]
            clientSocket.sendto(transaction_info.encode(), ('localhost', 15000))
