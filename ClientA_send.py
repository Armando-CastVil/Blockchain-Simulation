...
file_path = "balanceA.txt"
lock_path = "balanceA.txt.lock"
import time
...
def new_transaction():
    ...
    # this else take place if there are enough funds for the transaction
        ...
        clientSocket.sendto("clientA".encode(),('localhost',13000))
        #send the transaction to full node 1
        clientSocket.sendto(transaction_text.encode(),('localhost',13000))
...
def print_balance():
    ...
    b = open("BalanceF1.txt", "r")
    f1_balance = b.read()
    print("Full Node 1 Balance: " + f1_balance)
    b.close()
    f = open("balanceA.txt", "r")
    account_1 = f.readline()
    account_2 = f.readline()
